...
export const FaQs = () => {
    ...
    return (
        <div className='w-[95%] mx-auto'>
            {/* Categorías */}
            <div>
                <ul className="flex flex-row items-center justify-center gap-1 md:gap-6 lg:gap-32 xl:gap-36">
                    {Object.entries(iconosPorCategoria).map(([categoria, icono], index) => (
                        <li
                            className={`w-[250px] md:w-[320px] flex flex-col items-center border-2 py-4 rounded-2xl ${categoriaSeleccionada === categoria ? 'bg-zc-neutral-300' : ''}`}
                            key={index}
                            onClick={() => seleccionarCategoria(categoria)}
                            style={{cursor: 'pointer'}}
                        >
                            <span className="text-3xl md:text-4xl lg:text-5xl">{icono}</span>
                            <span className="text-sm md:text-lg lg:text-xl">{categoria}</span>
                        </li>
                    ))}
                </ul>
            </div>
            {/* Preguntas */}
            <div className='px-1 mx-auto md:px-6 lg:px-8 w-[99%] md:w-[99%] lg:w-[80%] mb-8'>
                {categoriaSeleccionada &&
                    <ul className='w-full md:w-[90%] lg:w-[80%] mx-auto mt-8 space-y-2'>
                        {data.preguntas_respuestas
                            .filter(item => item.categoria === categoriaSeleccionada)
                            .map(item => (
                                <div key={item.id}
                                     className='w-full transition-all duration-200 bg-white border border-gray-300 shadow-lg hover:bg-zc-fourth
                                     rounded-2xl'>
                                    <button type="button" className='w-full px-4 py-5 md:p-6' onClick={() => togglePregunta(item.id)}>
                                        <span className='flex flex-row items-center justify-between'>
                                            <p className='text-center text-lg lg:text-xl font-semibold text-black '>{item.pregunta}</p>
                                    ...
                                    {preguntaAbierta === item.id &&
                                        <div className='px-4 pb-4 md:px-6 sm:pb-6'>
                                            <p className='text-justify md:text-lg lg:text-xl'>{item.respuesta}</p>
                                        </div>
                                    }
                                </div>
                            ))}
